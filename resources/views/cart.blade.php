                ...
                <div class="sticky-content">
                    <div class="shopping-cart__totals">
                        ...
                        <table class="cart-totals">
                            <tbody>
                                ...
                                @if(Session::has('coupon') && Session::has('discounts'))
                                    @php
                                        // Convert session values to floats
                                        $discount = (float)(Session::get('discounts')['discount'] ?? 0);
                                        $subtotalAfterDiscount = (float)(Session::get('discounts')['subtotal'] ?? Cart::instance('cart')->subtotal());
                                        $tax = (float)(Session::get('discounts')['tax'] ?? 0);
                                        $total = (float)(Session::get('discounts')['total'] ?? Cart::instance('cart')->total());
                                    @endphp
                                    
                                    <tr>
                                        <th>Discount ({{ Session::get('coupon')['code'] ?? '' }})</th>
                                        <td class="text-danger">-${{ format_price($discount) }}</td>
                                    </tr>
                                    
                                    <tr>
                                        <th>Subtotal After Discount</th>
                                        <td>${{ format_price($subtotalAfterDiscount) }}</td>
                                    </tr>
                                @endif
                                ...
                                <tr>
                                    <th>VAT</th>
                                    <td>${{ format_price(Session::has('discounts') ? $tax : Cart::instance('cart')->tax()) }}</td>
                                </tr>
                                
                                <!-- Total Row -->
                                <tr class="cart-total">
                                    <th>Total</th>
                                    <td>${{ format_price(Session::has('discounts') ? $total : Cart::instance('cart')->total()) }}</td>
                                </tr>
                            </tbody>
                        </table>
                        ...
                        <div class="coupon-section mt-3">
                            ...
                            @else
                                <div class="applied-coupon alert alert-success py-2">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <span>
                                            <i class="fas fa-tag me-1"></i>
                                            {{ Session::get('coupon')['code'] }} 
                                            ({{ Session::get('coupon')['type'] === 'fixed' 
                                                ? '$'.format_price(Session::get('coupon')['value'])
